  ...
  <div class="content">
    <md-card>
      <md-card-header>
        ...
        <p class="category">Здесь вы можете получить ответ на отзыв, а так же другие оповещения</p>
      </md-card-header>

      <md-card-content>
        <div class="md-layout">
          <div class="md-layout-item md-small-size-100 md-size-100" v-if="getProfile.Alerts">
            <md-toolbar 
...
export default {
  ...
  methods: {
    ...
    async checkAlert(item) {
      ...
      await this.thisUserAlertCheck({id: item.id})
      .then(res => {
        //console.log(res)
      ...
      .catch(error => {
        ...
        this.$notify({
          message: `Оповещение не просмотрено! ${error}!`,
          icon: 'close',
          horizontalAlign: 'right',
          verticalAlign: 'top',
          type: 'danger'
